...
const appInsights = require('application');
appInsights.setup();
appInsights.start();
    ...
    if(!reqUrl || (!!reqUrl && (reqUrl == "" || reqUrl.toLowerCase() == "index.html"))){
        ...
        dbOperations.addRecord("index", function(){
            dbOperations.queryCount(function (visitCount){
                ...
                dom.window.document.getElementById("visitCount").innerHTML = "Total visits registered: " + visitCount;
                data = dom.serialize()
        ...
        res.writeHead(200, { 'Content-Type': '', '': data.length });
        res.write(data);
        res.end();
    }
